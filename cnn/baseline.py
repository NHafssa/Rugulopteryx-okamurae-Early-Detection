...
def baseline(data_director):
    """ Baseline model always returning 1.
    ...
    with torch.no_grad():
        for y, x in data_director.dataloader["tst"]:
            # move to device (x isn't used, but this keeps the pattern consistent)
            x = x.to(device)

            # y shape -> [B, 1]; dtype -> float; device -> device
            if y.ndim == 1:
                y = y.unsqueeze(1)
            y = y.to(device).float()

            # predictions: all ones, float, same device
            p = torch.ones((x.shape[0], 1), device=device, dtype=torch.float32)
...
def main():
    ...
    baseline(data_director=data_director)
    print(f"Done.")
